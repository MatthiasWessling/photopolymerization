...
\title{Background: chemistry and process ontology\\
of free-radical photopolymerization}
\author{Lab note for the Photopolymerization modelling campaign\\
\small Masters-level primer (not a results paper)}
...
\begin{abstract}
\noindent
This note is a \emph{background} for later modelling. It does not report new experiments and it does not describe software.
It states, in chemical and process-engineering language, \emph{what objects exist} when a multifunctional acrylate resin is turned into a polymer by light, and \emph{which of those objects must not be treated as the same thing}.
The intended reader has undergraduate physical chemistry and transport, but need not have worked on stereolithography or hydrogels.
The literature corpus that motivates the ontology is the set of papers collected for this campaign, especially the five local reports on first-principles kinetics \citep{Dobson2024}, grayscale digital-light processing \citep{Montgomery2022}, scanning-mask projection \citep{Meenakshisundaram2020}, photopolymerization in flow \citep{Slutzky2019}, and hydrogel network mechanics \citep{Zhu2020}, together with the oxygen, reciprocity, and gelation papers those reports rest on.
\end{abstract}
...
\section*{Evidence status of this document}

This note is a reading synthesis (\textbf{proposed} ontology).
It does not validate any numerical implementation.
Figures~\ref{fig:species}--\ref{fig:stack} are teaching cartoons.
Quantitative claims about a specific PEGDA formulation remain \textbf{still required} until they are taken from a named experiment or from a paper figure under the same recipe.
